...
import { generateDetailedFeedback, type FeedbackGenerationContext } from './feedback-actions';
...
export async function enrichCandidateWithFeedback(
    ...
    try {
        ...
        // Generate feedback (calls server action)
        const detailedFeedback = await generateDetailedFeedback(context, apiConfig);
        ...
        try {
            const fallbackFeedback = createFallbackDetailedFeedback(context);
            await updateCandidateFeedback(resultId, candidateId, {
                feedback: fallbackFeedback.summary,
            ...
            console.error(`❌ Failed to update fallback feedback for ${candidateId}:`, fallbackError);
        }
